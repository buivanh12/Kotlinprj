...
if (isset($_POST['login'])) {
    ...
    if (($pass !== $row['pass'])) {
        ...
        $_SESSION["tichdiem"] = $row['tichdiem'];
        ...
        $_SESSION["id_user"] = $row['id_user'];
        header("Location:../client/");
    } elseif ($row['type_user'] == "" && $email == $row['email'] && $pass == $row['pass']) {
        $_SESSION["loged"] = true;
...
$ngay_HD = "";
$id_gg = "";
if (isset($_POST['congdiem'])) {

    $id_congdiem = $_POST['id_user'];
    $cong = 0.01 * $_POST['tongtien'];
    $query = "UPDATE users SET tichdiem=tichdiem +? WHERE id_user=?";

    $stmt = $conn->prepare($query);

    $stmt->bind_param("si", $cong, $id_congdiem);

    $stmt->execute();

    header('location:../admin/HD.php');
}
if (isset($_POST['insertHD'])) {
    ...
    $id_user = '';
    $ten = '';
    list($id_user, $ten) = explode("-", $id_KH);

    $sdt_HD = $_POST['sdt_HD'];
    $ngay_HD = $_POST['ngay_HD'];
    $id_gg = $_POST['id_gg'];
    // Lấy giá trị ngày từ form

    // Chuyển đổi ngày thành kiểu văn bản (varchar)
    $vchar_ngay_HD = date('Y-m-d', strtotime($ngay_HD));

    // Sử dụng giá trị $vchar_ngay_HD theo nhu cầu của bạn

    $sql = "INSERT INTO `hoadon`( id_user,`TenKhacHang`,`SDT`,`Ngay`,id_gg) values (?,?,?,?,?)";
    $stmt = $conn->prepare($sql);
    var_dump($id_user, $ten, $sdt_HD, $vchar_ngay_HD, $id_gg);
    $stmt->bind_param("sssss", $id_user, $ten,  $sdt_HD, $vchar_ngay_HD, $id_gg);
    var_dump($stmt);
    $stmt->execute();
...
if (isset($_GET['editHD'])) {
    $id_brand = $_GET['editHD'];
    $query = "SELECT * FROM HoaDon WHERE MaHD=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $id_brand);
    $stmt->execute();
    $result = $stmt->get_result();
...
if (isset($_POST['updateHD'])) {
    ...
    $query = "UPDATE `hoadon` SET `TenKhacHang`=?,`SDT`=?,`Ngay`=?, id_gg=? WHERE MaHD=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ssssi",  $name_KH,  $sdt_HD, $vchar_ngay_HD, $id_gg, $id_brand);
    $stmt->execute();
    header('location:../admin/HD.php');
}
...
if (isset($_POST['insertSP_HD'])) {

    $id_HD = $_POST['id_HD'];
    $id_sanpham = $_POST['id_sanpham'];
    $Soluong = $_POST['Soluong'];

    // Kiểm tra xem có tồn tại mã hóa đơn và mã sản phẩm trong bảng `chitiet_hd` hay không
    $ktra = "SELECT SoLuong FROM `chitiet_hd` WHERE MaHD = ? AND id_product = ?";
    $stmt = $conn->prepare($ktra);
    $stmt->bind_param("ss", $id_HD, $id_sanpham);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        // Nếu tồn tại, thực hiện cập nhật số lượng
        $row = $result->fetch_assoc();
        $soLuongHienTai = $row['SoLuong'];
        $soLuongMoi = $soLuongHienTai + $Soluong;

        $updateSql = "UPDATE `chitiet_hd` SET SoLuong = ? WHERE MaHD = ? AND id_product = ?";
        $updateStmt = $conn->prepare($updateSql);
        $updateStmt->bind_param("sss", $soLuongMoi, $id_HD, $id_sanpham);
        $updateStmt->execute();
    } else {
        // Nếu không tồn tại, thực hiện thêm mới
        $insertSql = "INSERT INTO `chitiet_hd`(`MaHD`, `id_product`, `Soluong`) VALUES (?,?,?)";
        $insertStmt = $conn->prepare($insertSql);
        $insertStmt->bind_param("sss", $id_HD, $id_sanpham, $Soluong);
        $insertStmt->execute();
    }

    header("location:../admin/themHD.php?id_HD=$id_HD");
}




if (isset($_GET['deleteCT_HD'])) {
    $id_HD = $_GET['deleteCT_HD'];
    $id_sp = $_GET['id_sp'];
    $query = "DELETE FROM chitiet_hd WHERE MaHD=? and id_product=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ii", $id_HD, $id_sp);
    $stmt->execute();
    $result = $stmt->get_result();
...
if (isset($_POST['updateHD'])) {
    ...
    $query = "UPDATE `hoadon` SET `TenKhacHang`=?,`SDT`=?,`Ngay`=? WHERE MaHD=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sssi",  $name_KH,  $sdt_HD, $vchar_ngay_HD, $id_brand);
    $stmt->execute();
    header('location:../admin/HD.php');
}
